...
describe('hooks:install', () => {
  ...
  it('replaces hook placeholders in the template', () => {
    expect(replaceHookInString('{HOOK}', 'post-checkout')).to.equal('post-checkout');
    expect(replaceHookInString('# !/bin/bash\n node {HOOK}\nexit 0', 'post-merge')).to.equal('# !/bin/bash\n node post-merge\nexit 0');
    expect(replaceHookInString('{HOOK}:{HOOK}', 'post-checkout')).to.equal('post-checkout:post-checkout');
  });

  it('replaces info placeholders in the template', () => {
    expect(replaceHookInString('{INFO}')).to.equal(infoString);
    expect(replaceHookInString('# !/bin/bash\n# {INFO}\nexit 0')).to.equal(`# !/bin/bash\n# ${infoString}\nexit 0`);
    expect(replaceHookInString('{INFO}:{INFO}')).to.equal(`${infoString}:${infoString}`);
  });
